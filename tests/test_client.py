...
class TestClient(VCRTestCase):
    ...
    def test_update_list(self):
        akeneo = Client(self.base_url, self.client_id, self.secret, self.username, self.password)
    ...
    def test_update_very_long_list(self):
        ...
        if sys.version_info[0] == 3 and sys.version_info[1] < 6:
            logger.warning(
                "Disabling test test_update_very_long_list because of body"
    ...
    def test_product_search(self):
        if sys.version_info[0] == 3 and sys.version_info[1] < 6:
            logger.warning(
                "Disabling test test_product_search because of body"
                "matching issue with VCR with Python <3.6"
            )
            return
        akeneo = Client(self.base_url, self.client_id, self.secret, self.username, self.password)
        ...
        items = pool.fetch_list(args={"pagination_type": "search_after", "limit": 15})
    ...
    def test_get_resources(self):
        if sys.version_info[0] == 3 and sys.version_info[1] < 6:
            logger.warning(
                "Disabling test test_get_resources because of body"
                "matching issue with VCR with Python <3.6"
            )
            return
        akeneo = Client(self.base_url, self.client_id, self.secret, self.username, self.password)
    ...
    def test_client_auth_object(self):
        auth = Auth(self.base_url, self.client_id, self.secret, self.username, self.password)
        akeneo = Client(self.base_url, session=None, auth=auth)
        item = akeneo.products.fetch_item("1111111137")
