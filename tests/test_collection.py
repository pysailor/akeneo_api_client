    ...
    def test_create_delete_from_real_element(self):
        ...
        if False:
            ...
            pool = BasicResourcePool(
                urljoin("http://localhost:8088", "/api/rest/v1/", "products/"), session
            )
        else:
            pool = BasicResourcePool(
...
class TestCollectionMock(VCRTestCase):
    ...
    def test_fetch_item_from_invalid_pool(self):
        ...
        session.headers.update({"Content-Type": "application/json"})
        pool = BasicResourcePool(
            urljoin(self.base_url, "/api/rest/v1/", "products_invalid/"), session
        )
    ...
    def test_auto_loading_generator(self):
        ...
        for item in generator:
            i += 1
            self.assertNotEqual(generator.get_self_link(), generator.get_next_link())
            if i + 5 % 10 == 0:
